...
class GameDataService with ChangeNotifier {
  ...
  static Future<List<int>> getOwnedCharacterIds() async {
    try {
      final ownedCharacters = await getOwnedCharacters();
      return ownedCharacters.map((character) => character.id).toList();
    } catch (e) {
      return [1]; // الشخصية الافتراضية
    }
  }

  // ✅ دالة مساعدة للحصول على جميع الشخصيات
  static List<GameCharacter> getAllCharacters() {
    return GameCharacter.getAllCharacters();
  }

  // ✅ أضف هذه الدالة العامة للوصول إلى SharedPreferences
  static Future<void> setUnlockedLevels(List<int> levels) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('unlocked_levels', levels.join(','));
    } catch (e) {
      print('❌ خطأ في حفظ المستويات المفتوحة: $e');
    }
  }

  // ✅ دوال مساعدة للوصول الآمن إلى SharedPreferences
