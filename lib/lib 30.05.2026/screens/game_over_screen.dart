...
class _GameOverScreenState extends State<GameOverScreen> {
  ...
  Widget _buildAdsIcon(AppLocalizations l10n) {
    // استخدام الحالة الحقيقية من AdsRemovalService
    bool isAdsActive = _adsRemovalService.isActive;

    return Stack(
      children: [
        // أيقونة الإعلان من assets
        Image.asset(
        ...
        Positioned(
          ...
          child: Container(
            ...
            height: 16,
            decoration: BoxDecoration(
              color: isAdsActive ? Colors.green : Colors.red,
              shape: BoxShape.circle,
              border: Border.all(color: Colors.white, width: 1.5),
            ),
            child: Icon(
              isAdsActive ? Icons.check : Icons.warning,
              size: 12,
              color: Colors.white,
            ),
          ),
  ...
  void _openAdsRemovalOptions() {
    ...
    // إذا كانت الإعلانات معطلة، اعرض الحالة الحالية
    if (_adsRemovalService.isActive) {
      _showCurrentAdsStatus(l10n);
    } else {
      // إذا كانت مفعلة، اعرض خيارات الإزالة
      _showAdsRemovalOptionsWithUIService(l10n);
    }
  }

  // ✅ عرض خيارات إزالة الإعلانات
  void _showAdsRemovalOptionsWithUIService(AppLocalizations l10n) {
    showDialog(
      context: context,
  ...
  void _showCurrentAdsStatus(AppLocalizations l10n) {
    final adsStatus = _adsRemovalService.getAdsStatusInfo(
        isArabic: l10n.locale.languageCode == 'ar'
    );
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    if (adsStatus['remainingTime'] != null && adsStatus['remainingTime']!.isNotEmpty)
                      Padding(
                        padding: const EdgeInsets.only(top: 8),
                        child: Text(
                          '${l10n.remainingTime}: ${adsStatus['remainingTime']}',
                          style: const TextStyle(
                            color: Colors.white70,
                            fontSize: 14,
                          ),
